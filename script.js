...
function loadProfileInfo() {
    ...
    firebase.database().ref("/product_ids/" + uid).get().then((data) => {
        if (!data.exists) {
            document.getElementById("product-id").innerText = "not yet approved";
        } else {
            document.getElementById("product-id").innerText = data.val();
        }
        
        firebase.database().ref("/order_requests/" + data.val()).get().then((data1) => {
            var accInfo = data1.val();
            console.log("account info", accInfo);
            document.getElementById("name").innerText = accInfo.full_name;
            document.getElementById("address").innerText = accInfo.address_1;
            if (accInfo.address_2) {
                document.getElementById("address2").parentElement.style.display = "block";
            }
            document.getElementById("address2").innerText = accInfo.address_2;
            document.getElementById("city").innerText = accInfo.city;
        }).catch((reason) => {
            console.log("reason2", reason);
        })
        
    }).catch((reason) => {
        document.getElementById("product-id").innerText = "error";
...
async function fetchAttendanceData() {
    ...
    updateTable();
    var response = await axios.post("https://herecardattendancebackend.akze.repl.co/get_attendance/" + product_id);
    var ref = firebase.database().ref("/classes/" + product_id + "/students");
...
function updateAttendanceElement(element) {
    ...
    if (n !== undefined && n !== null) {
        ...
        var student = t[0];
        var date = t[1];
        firebase.database().ref("/classes/" + product_id + "/students/" + student + "/dates/" + date).update({state: n}).then((result) => {
